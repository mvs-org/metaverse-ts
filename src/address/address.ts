...
export class Address {
    static toBase58(address: string | Buffer, config: number | Network = DEFAULT_NETWORK, format: 'hex' | 'buffer' = 'buffer'): string | Buffer{
        ...
        const buffer = base58check.encode(typeof address === 'string' ? Buffer.from(address, 'hex') : address, toUInt8(version).toString('hex'))
        return format === 'hex' ? buffer.toString('hex') : buffer
    }
    static fromBase58(address: string, format: 'hex' | 'buffer' = 'buffer'): any{
