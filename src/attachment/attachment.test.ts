...
describe('Attachment', () => {
  ...
  describe('Coinstake', () => {
    const attachment = new AttachmentCoinstake()
    it('transform to buffer', () => {
      expect(attachment.toBuffer().toString('hex')).equal('01000000ffffffff')
    })
    it('transform to json', () => {
      expect(attachment.toJSON()).to.deep.equal({ type: ATTACHMENT_TYPE.COINSTAKE, version: ATTACHMENT_VERSION.DEFAULT })
    })
  })

  describe('Identifiable Token', ()=>{
  ...
  describe('Digital Identity', ()=>{
    describe('Did extension', () => {
      ...
      it('did enabled etp transfer', () => {
        const attachment = new AttachmentETPTransfer().setDid('foo', 'bar')
    ...
    describe('Invalid Avatar status', () => {
      it('invalid status', () => {
